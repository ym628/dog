#include "can.h"
u8 CAN_Send_Data[16], CAN_Receive_Data[16];
short slave_gyro[3], slave_accel[3]; // 接受从机发过来的陀螺仪数据
...
void CAN1_RX0_IRQHandler(void)
{
	...
	if (id == 0x201) // 通过ID区分两组数据
	{
		for (i = 0; i < 8; i++)
		{
			CAN_Receive_Data[i] = temp_rxbuf[i];
		}
	}
	else if (id == 0x202) // 通过ID区分两组数据
	{
		for (i = 8; i < 16; i++)
		{
			CAN_Receive_Data[i] = temp_rxbuf[i - 8];
		}
	}
	CAN_ReceiveData(); // 对接收到的数据进行处理
}
#endif
...
{
	u8 CAN_SENT[8], i;
	// 数据分开两组发送，一次发送8位，通过ID来区分两组数据
	for (i = 0; i < 8; i++)
	{
		CAN_SENT[i] = CAN_Send_Data[i];
	}
	CAN1_Send_Num(0x101, CAN_SENT); // 第一组ID是0x101

	for (i = 0; i < 8; i++)
	{
		CAN_SENT[i] = CAN_Send_Data[i + 8];
	}
	CAN1_Send_Num(0x102, CAN_SENT); // 第二组ID是0x102
}
...
{
	// CAN一次发送8位数据，这里将数据拆分成两组来发送
	CAN_Send_Data[0] = EncoderTarget.M5 >> 8; // 这里是将从机的四个电机的目标编码器数值发送过去
	CAN_Send_Data[1] = EncoderTarget.M5;
	CAN_Send_Data[2] = EncoderTarget.M6 >> 8;
	CAN_Send_Data[3] = EncoderTarget.M6;
	CAN_Send_Data[4] = EncoderTarget.M7 >> 8;
	CAN_Send_Data[5] = EncoderTarget.M7;
	CAN_Send_Data[6] = EncoderTarget.M8 >> 8;
	CAN_Send_Data[7] = EncoderTarget.M8;

	CAN_Send_Data[8] = RC_Velocity >> 8; // 电池电压的高低8位
	CAN_Send_Data[9] = RC_Velocity;
	CAN_Send_Data[10] = (APP_ON_Flag << 3) | (PS2_ON_Flag << 2) | (Remote_ON_Flag << 1) | USART_ON_FLAG; // 8:APP,4:PS2,2:航模遥控,1:串口（ROS）
	CAN_Send_Data[11] = armingFlags;																	 // 四足的状态
	CAN_Send_Data[12] = 0;
	CAN_Send_Data[13] = 0;
	CAN_Send_Data[14] = 0;
	CAN_Send_Data[15] = 0;
...
{
	slave_gyro[0] = (short)(CAN_Receive_Data[0] << 8) + CAN_Receive_Data[1];
	slave_gyro[1] = (short)(CAN_Receive_Data[2] << 8) + CAN_Receive_Data[3];
	slave_gyro[2] = (short)(CAN_Receive_Data[4] << 8) + CAN_Receive_Data[5];

	slave_accel[0] = (short)(CAN_Receive_Data[8] << 8) + CAN_Receive_Data[9];
	slave_accel[1] = (short)(CAN_Receive_Data[10] << 8) + CAN_Receive_Data[11];
	slave_accel[2] = (short)(CAN_Receive_Data[12] << 8) + CAN_Receive_Data[13];
}
