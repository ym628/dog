#include "can.h"
u8 can_sent[16], can_receive[16];	  // CAN接收和发送数组
u8 control_mode = 0, armingFlags = 0; // 四足相关标志位
short RC_Velocity;					  // 四足速度
/**************************************************************************
函数功能：CAN发送任务函数
入口参数：无
返回  值：无
...
{
	can_sent[0] = gyro[0] >> 8; // 三轴角速度
	can_sent[1] = gyro[0];		// 拆分成高低8位再发送
	can_sent[2] = gyro[1] >> 8;
	can_sent[3] = gyro[1];
	can_sent[4] = gyro[2] >> 8;
	can_sent[5] = gyro[2];

	can_sent[8] = accel[0] >> 8; // 三轴加速度
	can_sent[9] = accel[0];		 // 拆分成高低8位再发送
	can_sent[10] = accel[1] >> 8;
	can_sent[11] = accel[1];
	can_sent[12] = accel[2] >> 8;
	can_sent[13] = accel[2];
}
...
void CAN_SEND(void)
{
	...
	for (i = 0; i < 8; i++)
	{
		CAN_SENT[i] = can_sent[i];
	}
	CAN1_Send_Num(0x201, CAN_SENT);

	for (i = 0; i < 8; i++)
	{
		CAN_SENT[i] = can_sent[i + 8];
	}
	CAN1_Send_Num(0x202, CAN_SENT);
}
/**************************************************************************
 *  函数功能：CAN1初始化
 *
...
void USB_LP_CAN1_RX0_IRQHandler(void)
{
	...
	if (id == 0x101) // 通过ID区分两组数据
	{
		for (i = 0; i < 8; i++)
		{
			can_receive[i] = temp_rxbuf[i];
		}
	}
	else if (id == 0x102) // 通过ID区分两组数据
	{
		for (i = 8; i < 16; i++)
		{
			can_receive[i] = temp_rxbuf[i - 8];
		}
	}
	can_data_processing(); // 接收到数据后对数据进行处理
}
#endif
...
{
	MOTOR_A.Target = (short)(can_receive[0] << 8) + can_receive[1]; // 四路电机的目标编码器值
	MOTOR_B.Target = (short)(can_receive[2] << 8) + can_receive[3];
	MOTOR_C.Target = (short)(can_receive[4] << 8) + can_receive[5];
	MOTOR_D.Target = (short)(can_receive[6] << 8) + can_receive[7];
	RC_Velocity = (can_receive[8] << 8) + can_receive[9]; // 小车速度
	control_mode = can_receive[10];						  // 四足控制模式
	armingFlags = can_receive[11];						  // 四足状态
}
