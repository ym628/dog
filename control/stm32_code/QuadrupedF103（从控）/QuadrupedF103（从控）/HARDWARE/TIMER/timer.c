...
void TIM1_Cap_Init(u16 arr, u16 psc)
{
	...
	GPIO_ResetBits(GPIOA, GPIO_Pin_8 | GPIO_Pin_9 | GPIO_Pin_11); // PA8 9 11  输入   下拉
...
void TIM1_CC_IRQHandler(void)
{
	...
	if ((TIM1CH3_CAPTURE_STA & 0X80) == 0) // 还未成功捕获
	{
		if (TIM_GetITStatus(TIM1, TIM_IT_CC3) != RESET) // 捕获3发生捕获事件
		{
			...
			{
				TIM1CH3_CAPTURE_DOWNVAL = TIM_GetCapture3(TIM3); // 记录下此时的定时器计数值
